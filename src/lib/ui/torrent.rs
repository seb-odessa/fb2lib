...
const ARCHIVE_FILE: &'static str = "archive.zip";
const ARCHIVE_FILE_HELP: &'static str = "The archive.zip with books";
const TORRENT_FILE: &'static str = "file.torrent";
const TORRENT_FILE_HELP: &'static str = "Torrent file name points to the archive.zip";
...
pub fn add<'a, 'b>(app: App<'a, 'b>) -> App<'a, 'b> {
    let db = Arg::with_name(DB_FILE).help(DB_FILE_HELP).required(true);
    let archive = Arg::with_name(ARCHIVE_FILE).help(ARCHIVE_FILE_HELP).required(true);
    let torrent = Arg::with_name(TORRENT_FILE).help(TORRENT_FILE_HELP).required(true);
...
pub fn handle<'a>(arg: &ArgMatches<'a>) -> Fb2Result<()> {
    ...
    match arg.subcommand() {
        (LOAD, Some(arg)) => {
            let torrent = arg.value_of(TORRENT_FILE).unwrap_or("");
            torrent_load(database, torrent)
        },
        (CHECK, Some(arg)) => {
            let archive = arg.value_of(ARCHIVE_FILE).unwrap_or("");
