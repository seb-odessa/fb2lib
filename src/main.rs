...
fn main() {
    ...
    let archive = Arg::with_name(ARCHIVE)
        ...
        .index(1)
        .required(false);
    let book = Arg::with_name(FILE)
        .help("file.fb2 in archive")
        .index(1)
        .required(true);
    let xml = Arg::with_name(XML)
        .help("Xml file in FB2 format")
        .index(1)
        .required(true);
    ...
    let cmd_check = SubCommand::with_name(CMD_CHECK)
        ...
        .arg(quiet);
    let cmd_show =
        SubCommand::with_name(CMD_SHOW).about("Request to extract and print some kind of content");
    let cmd_show_xml = SubCommand::with_name(CMD_XML)
        .about("Print XML content of the fb2 description")
        .arg(book.clone());
    let cmd_show_fb2 = SubCommand::with_name(CMD_FB2)
    ...
    let result = match app.subcommand() {
        ...
        (CMD_CHECK, Some(cmd)) => do_check(&archive, cmd.occurrences_of(QUIET) != 0),
